...
public class DAO {
	...
	public int getUpdate(VO vo) {
		int result = 0 ;
		String sql = "update guesbook2 set name:? , subject=? , email=? , file_name=? , content=? where isx=?";
		result = jdbcTemplate.update(sql, vo.getName(),vo.getSubject(),vo.getEmail(),vo.getFile_name(),vo.getContent(),vo.getIdx());
		return result;
	}
}
